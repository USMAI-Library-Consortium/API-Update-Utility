...
def verify_response_content(api_resource: ApiResource, test_xpath: str):
    ...
    if api_resource.status == "pending" and api_resource.xml_from_get_request:
        ...
        if len(xpath_results) == 0:
            logging.warning(f"Malformed GET response from resource with ID {
                            api_resource.identifier}. URL: {api_resource.api_url}")
            api_resource.status = "failed"
        else:
            logging.debug(f"Verification for resource {
                            api_resource.identifier} successful.")
